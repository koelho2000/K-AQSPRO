...
const SystemPage: React.FC<SystemPageProps> = ({ systemType, project, setProject, results = [], isDirty, onRunSimulation }) => {
  ...
  const technicalKPIs = useMemo(() => {
    ...
    let failureHours = results.filter(r => r.demand_L > 0 && r.temp_tank < designSetpoint).length;
    if (results.length === 0) failureHours = 0;
  ...
  const addEquipment = (type: Equipment['type']) => {
    ...
    switch(type) {
      ...
      case 'BOILER': name = 'Caldeira Gás Condensação'; power = 24; efficiency = 1.05; break;
  ...
  return (
    <div className="p-8 max-w-7xl mx-auto space-y-10 animate-in fade-in duration-500 pb-24">
      ...
      <div className="grid grid-cols-1 md:grid-cols-4 gap-6">
         ...
         <div className={`p-6 rounded-3xl border shadow-sm transition-all ${results.length > 0 && technicalKPIs.failureHours > 0 ? 'bg-red-50 border-red-200' : 'bg-green-50 border-green-200'}`}>
            <div className={`flex items-center gap-2 text-[10px] font-black uppercase tracking-widest mb-2 ${results.length > 0 && technicalKPIs.failureHours > 0 ? 'text-red-600' : 'text-green-600'}`}>
              <Clock size={14}/> {results.length > 0 && technicalKPIs.failureHours > 0 ? 'Horas < Setpoint' : 'Status Conforto'}
            </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
        ...
        <div className="lg:col-span-2 space-y-4">
          <div className="flex justify-between items-center mb-4"><h3 className="font-black text-slate-900 uppercase text-xs tracking-widest">Geração e Apoio Térmico</h3><div className="flex gap-2">{['HP', 'BOILER', 'SOLAR'].map(type => (<button key={type} onClick={() => addEquipment(type as any)} className="px-3 py-2 bg-slate-100 text-slate-700 rounded-lg font-bold text-[10px] hover:bg-slate-200 uppercase tracking-tight">{type}</button>))}</div></div>
          {system.equipments.map((eq, i) => (
            <div key={i} className="bg-white p-6 rounded-[32px] shadow-sm border border-slate-200 flex items-start gap-6 border-l-8 transition-all hover:shadow-md" style={{ borderLeftColor: eq.type === 'SOLAR' ? '#f59e0b' : (eq.type === 'HP' ? '#3b82f6' : '#ef4444') }}>
          ...
          <div className="grid grid-cols-1 gap-8">
            <div className="bg-white p-8 rounded-[40px] border border-slate-200 shadow-sm h-[500px]">
              <div className="flex justify-between items-center mb-10">
                <h4 className="text-[10px] font-black text-slate-400 uppercase tracking-widest flex items-center gap-2"><Droplets size={14} className="text-blue-500"/> Caudal de Consumo (L/h) vs Temperatura Atingida (ºC)</h4>
